...
def extract_named_entities(client, documents: list):
    response = client.recognize_entities(documents = documents)[0]

    if not response.is_error: return response.entities
...
def main():
    ...
    named_entities = extract_named_entities(named_entities_extractor_client, documents)
    breaker()
    print("Entities : \n")
